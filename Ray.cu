 ...
 *
 * This file is part of CudaKinect
 * <https://github.com/favreau/CudaKinect>
 *
...
{                                                                         \
    ...
    __color = ((int)(__a * 255.0f) << 24) | ((int)(__b * 255.0f) << 16) |      \
              ((int)(__g * 255.0f) << 8) | ((int)(__r * 255.0f) << 0);         \
  \
}

// --------------------------------------------------------------------------------
#define length(__vl) sqrt(__vl.x *__vl.x + __vl.y * __vl.y + __vl.z * __vl.z)

// --------------------------------------------------------------------------------
#define fastLength(__v) __v.x *__v.x + __v.y *__v.y + __v.z *__v.z
...
#define reflect(incident, normal, reflected)                                   \
  {                                                                            \
    ...
    float __d = 2.0f * __a;                                                    \
    set(reflected, incident.x - __d * normal.x, incident.y - __d * normal.y,   \
        incident.z - __d * normal.z);                                          \
  }
...
// --------------------------------------------------------------------------------
#define isNull(v) (v.x == 0.0f && v.y == 0.0f && v.z == 0.0f)
...
--------------------------------------------------------------------------------
*/
#define sphereIntersection(__primitive, __origin, __ray, __O_C,                \
                           __intersection, __collision)                        \
  float si_A =                                                                 \
      2.0f * (__ray.x * __ray.x + __ray.y * __ray.y + __ray.z * __ray.z);      \
  if (si_A == 0.0f)                                                            \
    si_A = 0.0001f;                                                            \
  float si_B =                                                                 \
      2.0f * (__O_C.x * __ray.x + __O_C.y * __ray.y + __O_C.z * __ray.z);      \
  float si_C = __O_C.x * __O_C.x + __O_C.y * __O_C.y + __O_C.z * __O_C.z -     \
               __primitive.radius * __primitive.radius;                        \
  float si_radius = si_B * si_B - 2.0f * si_A * si_C;                          \
  float si_t1 = (-si_B - sqrt(si_radius)) / si_A;                              \
  bool si_b1 = (si_t1 > 0.0f);                                                 \
  set(__intersection, __origin.x + si_t1 * __ray.x,                            \
      __origin.y + si_t1 * __ray.y, __origin.z + si_t1 * __ray.z);             \
  float si_t2 = (-si_B + sqrt(si_radius)) / si_A;                              \
  bool si_b2 = (si_t2 > 0.0f);                                                 \
  if (si_b2) {                                                                 \
    float3 si_intersection2;                                                   \
    set(si_intersection2, __origin.x + si_t2 * __ray.x,                        \
        __origin.y + si_t2 * __ray.y, __origin.z + si_t2 * __ray.z);           \
    if (si_b1) {                                                               \
      float3 si_o1, si_o2;                                                     \
...
{                                                                         \
    ...
    case ptSphere:                                                             \
      subtract(__intersection, __primitive.center, __normal);                  \
      break;                                                                   \
    case ptTriangle:                                                           \
      set(__normal, 0.0f, 0.0f, -1.0f);                                        \
      break;                                                                   \
    case ptCheckboard:                                                         \
      set(__normal, 0.0f, 1.0f, 0.0f);                                         \
      break;                                                                   \
    }                                                                          \
  \
}
...
--------------------------------------------------------------------------------
*/
#define checkboardIntersection(__primitive, __origin, __ray, __intersection,   \
                               __collision)                                    \
  \
{                                                                         \
    switch (__primitive.type) {                                                \
...
--------------------------------------------------------------------------------
*/
#define shadow(__primitives, __nbVisiblePrimitives, __lampCenter, __origin,    \
               __objectId, __collision)                                        \
  \
{                                                                         \
    float3 shadow_intersection;                                                \
    float3 shadow_O_L;                                                         \
    set(shadow_O_L, __lampCenter.x - __origin.x, __lampCenter.y - __origin.y,  \
        __lampCenter.z - __origin.z);                                          \
    int cptPrimitives = 0;                                                     \
    while (!__collision && cptPrimitives < __nbVisiblePrimitives) {            \
      if (cptPrimitives != __objectId &&                                       \
          __primitives[cptPrimitives].type == ptSphere) {                      \
        ...
        if (shadow_nd < -0.99f) {                                              \
          sphereIntersection(__primitives[cptPrimitives], __origin,            \
                             shadow_O_L, shadow_O_C, shadow_intersection,      \
                             shadow_collision);                                \
 ...
 * --------------------------------------------------------------------------------
 */
#define getObjectColorAtIntersection(__primitive, __intersection,              \
                                     __colorAtIntersection)                    \
  \
{                                                                         \
    ...
    case ptCamera: {                                                           \
      ...
      if (x >= 0 && x < 640 && y >= 0 && y < 480) {                            \
        int index = (y * 640 + x) * 4;                                         \
        set(__colorAtIntersection, float(d_BkGround[index]) / 256.0f,          \
            float(d_BkGround[index + 1]) / 256.0f,                             \
            float(d_BkGround[index + 2]) / 256.0f);                            \
      } else {                                                                 \
        __colorAtIntersection = __primitive.color;                             \
      }                                                                        \
      break;                                                                   \
...
// --------------------------------------------------------------------------------
#define colorFromObject(__primitives, __nbVisiblePrimitives, __lamps,          \
                        __rayDir, __NormalToSurface, __intersection,           \
                        __objectId, __color)                                   \
  \
{                                                                         \
    float cfo_lambert;                                                         \
    ...
    for (int cfo_cptLamps = 0; cfo_cptLamps < _dNbLamps; cfo_cptLamps++) {     \
      ...
      shadow(__primitives, __nbVisiblePrimitives,                              \
             __lamps[cfo_cptLamps].center, __intersection, __objectId,         \
             cfo_collision);                                                   \
      if (!cfo_collision) {                                                    \
        float3 cfo_lightRay;                                                   \
    ...
    float3 cfo_color;                                                          \
    getObjectColorAtIntersection(__primitives[__objectId], __intersection,     \
                                 cfo_color);                                   \
    set(__color, cfo_color.x *cfo_lampsColor.x, cfo_color.y *cfo_lampsColor.y, \
 ...
 * --------------------------------------------------------------------------------
 */
#define quickSortPrimitives(__primitives, __nbPrimitives, __origin, __ray,     \
                            __closestObjectId)                                 \
  \
{                                                                         \
    __closestObjectId = 0;                                                     \
    ...
    float3 qsp_O_C;                                                            \
    float qsp_nd;                                                              \
    for (int qsp_cptObjects = 0; qsp_cptObjects < __nbPrimitives;              \
         qsp_cptObjects++) {                                                   \
      if (qsp_cptObjects != lr_newObjectId) {                                  \
        ...
        if (qsp_nd > 0.99999f) {                                               \
          qsp_distance =                                                       \
              length(qsp_O_C) - __primitives[qsp_cptObjects].radius;           \
...
#define newIntersectionWithObjects(__primitives, __nbPrimitives, __origin,     \
                                   __orientation, __closestObjectId,           \
                                   __closestIntersection, __intersections)     \
  \
{                                                                         \
    __intersections = false;                                                   \
    float3 iwo_ray;                                                            \
    subtract(__orientation, __origin, iwo_ray);                                \
    quickSortPrimitives(__primitives, __nbPrimitives, __origin, iwo_ray,       \
                        __closestObjectId);                                    \
    switch (__primitives[__closestObjectId].type) {                            \
    case ptSphere: {                                                           \
      float3 iwo_O_C;                                                          \
      subtract(__origin, __primitives[__closestObjectId].center, iwo_O_C);     \
      sphereIntersection(__primitives[__closestObjectId], __origin, iwo_ray,   \
                         iwo_O_C, __closestIntersection, __intersections);     \
      break;                                                                   \
    }                                                                          \
    case ptTriangle: {                                                         \
      /*__intersections = TriangleIntersection(                                \
       * __primitives[__closestObjectId], __origin, __orientation,             \
       * __closestIntersection );*/                                            \
      break;                                                                   \
    }                                                                          \
    case ptCheckboard:                                                         \
    case ptCamera: {                                                           \
      checkboardIntersection(__primitives[__closestObjectId], __origin,        \
                             iwo_ray, __closestIntersection, __intersections); \
      break;                                                                   \
    }                                                                          \
    }                                                                          \
  \
 ...
 * --------------------------------------------------------------------------------
 */
#define intersectionWithObjects(__primitives, __nbPrimitives, __origin,        \
                                __orientation, __closestObjectId,              \
                                __closestIntersection, __intersections)        \
  \
{                                                                         \
    ...
    float3 iwo_ray;                                                            \
    subtract(__orientation, __origin, iwo_ray);                                \
    for (int iwo_cptObjects = 0; iwo_cptObjects < __nbPrimitives;              \
         iwo_cptObjects++) {                                                   \
      if (iwo_cptObjects != lr_newObjectId) {                                  \
        ...
        if (iwo_a < 0.0f) {                                                    \
          ...
          case ptSphere: {                                                     \
            sphereIntersection(__primitives[iwo_cptObjects], __origin,         \
                               iwo_ray, iwo_O_C, iow_intersection,             \
                               iwo_intersec);                                  \
            break;                                                             \
          }                                                                    \
          case ptTriangle: {                                                   \
            /*iwo_intersec = TriangleIntersection(                             \
             * __primitives[iwo_cptObjects], __origin, __orientation,          \
             * iow_intersection );*/                                           \
            break;                                                             \
          }                                                                    \
          case ptCheckboard:                                                   \
          case ptCamera: {                                                     \
            checkboardIntersection(__primitives[iwo_cptObjects], __origin,     \
                                   iwo_ray, iow_intersection, iwo_intersec);   \
            break;                                                             \
          }                                                                    \
          }                                                                    \
          if (iwo_intersec) {                                                  \
            float iwo_distance = (__origin.x - iow_intersection.x) *           \
                                     (__origin.x - iow_intersection.x) +       \
                                 (__origin.y - iow_intersection.y) *           \
                                     (__origin.y - iow_intersection.y) +       \
                                 (__origin.z - iow_intersection.z) *           \
                                     (__origin.z - iow_intersection.z);        \
            if (iwo_distance < iwo_minDistance) {                              \
              iwo_minDistance = iwo_distance;                                  \
              __closestObjectId = iwo_cptObjects;                              \
              set(__closestIntersection, iow_intersection.x,                   \
                  iow_intersection.y, iow_intersection.z);                     \
 ...
 * intersection float3 and will also compute the shadows. The resulted color is
 * stored in result. The first parameter is the closest object to the
 * intersection (following the ray). It can be considered as a light source if
 * its inner light rate is > 0.
 *  ------------------------------------------------------------------------------------------
 */
#define launchRay(__primitives, __nbVisiblePrimitives, __lamps, __viewPos,     \
                  __viewRay, __intersectionColor)                              \
  \
{                                                                         \
    ...
    while (lr_iteration < nbIterations && lr_carryon) {                        \
      intersectionWithObjects(__primitives, __nbVisiblePrimitives,             \
                              lr_newOrigin, lr_newOrientation,                 \
                              lr_closestObjectId, lr_closestIntersection,      \
                              lr_carryon);                                     \
      if (lr_carryon) {                                                        \
        float3 lr_O_I;                                                         \
        subtract(__viewPos, lr_closestIntersection, lr_O_I);                   \
        float3 lr_normalToSurface;                                             \
        getNormalVector(__primitives[lr_closestObjectId],                      \
                        lr_closestIntersection, lr_normalToSurface);           \
        reflect(lr_O_I, lr_normalToSurface, lr_reflection);                    \
        float3 lr_closestOrientation;                                          \
        subtract(lr_closestIntersection, lr_reflection,                        \
                 lr_closestOrientation);                                       \
        float3 lr_objectColor;                                                 \
        colorFromObject(__primitives, __nbVisiblePrimitives, __lamps,          \
                        lr_closestOrientation, lr_normalToSurface,             \
                        lr_closestIntersection, lr_closestObjectId,            \
                        lr_objectColor);                                       \
    ...
    makeColor(lr_intersectionColor.x, lr_intersectionColor.y,                  \
              lr_intersectionColor.z, 0.0f, __intersectionColor);              \
  \
}
 ...
 * --------------------------------------------------------------------------------
 */
__global__ void init_rays_kernel(float3 viewPos, float3 P1, float3 P4,
                                 float3 d12, float3 d34, float2 imageSize,
                                 float3 *rays) {
  // Compute the index
 ...
 * --------------------------------------------------------------------------------
 */
__global__ void kernel(float3 viewPos, float2 imageSize, Primitive *primitives,
                       Lamp *lamps, int nbVisiblePrimitives, int nbLamps,
                       float3 *rays, unsigned int *d_output,
                       GLubyte *d_BkGround) {
  // Compute the index
  unsigned int x = __umul24(blockIdx.x, blockDim.x) + threadIdx.x;
  unsigned int y = __umul24(blockIdx.y, blockDim.y) + threadIdx.y;
  ...
  launchRay(primitives, nbVisiblePrimitives, lamps, viewPos, rays[index],
            d_output[index]);
}
...
extern "C" void initCuda(const unsigned char *h_volume, cudaExtent volumeSize,
  ...
  copyParams.srcPtr = make_cudaPitchedPtr(
      (void *)h_volume, volumeSize.width * sizeof(unsigned char),
  ...
  // Allocate memory
  checkCudaErrors(
      cudaMalloc((void **)&_dBkGround, 640 * 480 * sizeof(DWORD) * 4));
  checkCudaErrors(
      cudaMalloc((void **)&_dPrimitives, _dNbPrimitives * sizeof(Primitive)));
  checkCudaErrors(cudaMalloc((void **)&_dLamps, _dNbLamps * sizeof(Lamp)));
  checkCudaErrors(
      cudaMalloc((void **)&_dRays, width * height * sizeof(float3)));
}
...
extern "C" void render_kernel(unsigned int *d_output, int imageW, int imageH,
                              ...
                              float time, NUI_SKELETON_FRAME SkeletonFrame) {
  ...
  set(primitives[2].center, -15.0f * cosf(time),
      -10.0f + 10.0f * abs(cosf(time)), -15.0f * sinf(time));
  ...
  set(lamps[0].center, 0.0f, 200.0f, -200.0f);
  lamps[0].intensity = 2.0f;
  set(lamps[0].color, 0.5f, 0.5f, 0.5f);
  set(lamps[1].center, -100.0f * sinf(time / 2.2f), 200.0f,
      -100.0f * cosf(time / 3.5f));
  lamps[1].intensity = 1.0f;
  set(lamps[1].color, 1.0f, 1.0f, 1.0f);

  checkCudaErrors(cudaMemcpy(_dPrimitives, primitives,
                             nbVisiblePrimitives * sizeof(Primitive),
                             cudaMemcpyHostToDevice));
  checkCudaErrors(cudaMemcpy(_dLamps, lamps, _dNbLamps * sizeof(Lamp),
                             cudaMemcpyHostToDevice));
#if 0
  ...
  dim3 block(13, 13, 1);
  dim3 grid(unsigned int(imageSize.x / block.x),
            unsigned int(imageSize.y / block.y), 1);
  ...
  kernel<<<grid, block>>>(eye, imageSize, _dPrimitives, _dLamps,
                          nbVisiblePrimitives, _dNbLamps, _dRays, d_output,
                          _dBkGround);
}
